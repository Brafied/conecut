...
def get_conecut_data(arguments):
    logger.info(f"Loading cached activation differences for {arguments.model_id} on RewardBench...")
...
def augment_redundancy(arguments, activation_differences, augmentation_activation_differences):
    ...
    for i in range(start_index, len(combined_activation_differences)):
        ...
        non_redundant_examples_mask = np.ones(len(combined_activation_differences), dtype=bool)
    ...
    logger.info(f"Percent redundant: {(len(redundant_examples) / len(combined_activation_differences)) * 100:.2f}%.")
...
def main():
    ...
    activation_differences = get_conecut_data(arguments)

    augmentation_activation_differences = get_augmentation_data(arguments)

    augment_redundancy(arguments, activation_differences, augmentation_activation_differences)
